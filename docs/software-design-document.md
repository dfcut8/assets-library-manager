...
The application is implemented in Go 1.26 or newer and distributed as one application executable. The executable embeds the web UI, HTMX library, styles, templates, and database migrations. Runtime state consists of an optional-on-first-run JSON configuration file, a SQLite database, an incoming directory, and a processed-assets directory located beside the executable. Missing runtime state is bootstrapped automatically when doing so cannot orphan managed data. AI processing uses a host-installed Codex CLI authenticated with the user's ChatGPT subscription; the application does not accept, store, or call OpenAI with an API key.
...
- Use subscription-backed Codex image analysis to generate a title, description, primary asset type, style, confidence, and rich tags.
...
- Providers other than a ChatGPT-authenticated Codex App Server.
...
| Semantic metadata | Descriptive values generated by Codex and subsequently editable by the user. |
| Codex App Server | A host process started through the Codex CLI and controlled over its stdio JSON-RPC protocol. |
...
flowchart LR
    ...
    App -->|"stdio JSON-RPC"| Codex["Host Codex App Server"]
    Codex -->|"ChatGPT subscription usage"| OpenAI["OpenAI models"]
    App -->|"Reveal managed file"| FileManager["Operating-system file manager"]
```

The user must understand that Codex sends asset image data to OpenAI for categorization and consumes the signed-in ChatGPT account's Codex allowance. The application and App Server process run locally, but AI processing is neither offline nor self-hosted.
...
| FR-003 | Serve the existing catalog even when Codex is missing, signed out, misconfigured, rate-limited, or unavailable. |
...
| FR-009 | Generate schema-constrained semantic metadata through a ChatGPT-authenticated Codex App Server. |
...
| ID | Requirement |
| --- | --- |
| NFR-001 | Produce one Go application executable with no required application-side Node.js, CDN, browser extension, or CGO dependency; document the separate host Codex CLI prerequisite. |
...
| NFR-007 | Never read, copy, persist, log, or expose Codex authentication tokens or authorization data. |
...
| Codex analyzer | Own the App Server subprocess, verify ChatGPT authentication, build image turns, request structured output, classify retryable errors, validate responses, and retain provenance. |
...
- Resolve the configured Codex command with `exec.LookPath`, invoke it directly without a shell, use stdio JSON-RPC, and terminate every child process during preflight completion or application shutdown.
...
internal/codex/                 App Server process, protocol, authentication preflight, and response validation
...
- `codex.command` defaults to `codex` and is resolved through the host `PATH`. An explicit executable path is permitted when the CLI is not on `PATH`; the complete value is always treated as one literal command path rather than parsed as a shell command line.
- `codex.reasoning_effort` accepts `low`, `medium`, `high`, or `xhigh`; App Server model discovery or the analysis turn may reject a model-specific mismatch.
- `codex.startup_timeout_seconds` bounds command resolution, process startup, protocol initialization, and `account/read` during preflight.
- `codex.turn_timeout_seconds` is the total deadline for one future analysis operation, including attempts and backoff.
- Retry attempts include the initial attempt and must be between 1 and 10.
- The application has no OpenAI API-key setting and ignores `OPENAI_API_KEY`. Codex owns its cached ChatGPT credentials; the application must not read `auth.json`, an OS credential store, access tokens, refresh tokens, or authorization headers.

The sample model is not a permanent compatibility promise. The model and effort remain configuration values so they can change without rebuilding the application, but the runtime must use `model/list` to verify the selected model supports image input before processing. The application always supplies the locally bounded analysis rendition rather than an unbounded managed original. The [Codex App Server protocol](https://learn.chatgpt.com/docs/app-server) documents model discovery, local image input, per-turn output schemas, authentication, and rate-limit reporting.
...
sequenceDiagram
    ...
    participant AI as Codex analyzer
...
6. Send the analysis rendition to the managed Codex App Server as `localImage` input and validate schema-constrained semantic output.
...
- Provider and model.
- Reasoning effort and image-input mode when exposed by the App Server.
...
### 10.1 Host and authentication contract

- The host must provide a compatible Codex CLI command, normally `codex`, and a cached ChatGPT login created by `codex login`.
- Asset Library Manager starts and owns `codex app-server --listen stdio://`; it does not require the Codex desktop app to be open and does not connect to an arbitrary pre-existing TCP listener.
- Communicate over newline-delimited JSON-RPC on stdio. Send exactly one `initialize` request per connection, wait for success, then send the `initialized` notification before any other request.
- Verify `account/read.account.type == "chatgpt"`. A null account is signed out; `apiKey` is explicitly not subscription-backed; other account types are unsupported in v1.
- The application never implements OAuth, reads Codex credential storage, or receives cached tokens. Codex owns credential persistence and automatic refresh.
- Use `account/rateLimits/read` before scheduling a batch and after rate-limit notifications. When a relevant window is exhausted, stop reserving new items, retain sources, and show the reset time when the protocol supplies it.
...
- Validate turn completion, refusal/error state, output schema, field lengths, enum values, confidence range, and tag counts before accepting a result.
- Persist model, prompt/schema versions, Codex thread/turn identifiers when safe and useful, bounded usage/rate-limit information, and normalized output. Do not persist unrestricted event streams or model reasoning.

### 10.3 Retry policy

`turn_timeout_seconds` is one deadline for the complete analysis operation, including all attempts, event reads, validation, and backoff. Every JSON-RPC line and decoded message is size-limited, the number of irrelevant notifications tolerated while awaiting a response is bounded, and the child process is canceled and joined on timeout or shutdown.

Retry only transient failures such as App Server interruption and eligible upstream failures. Respect the rate-limit reset time when supplied; otherwise use exponential backoff with full jitter from `initial_retry_delay_ms`. Stop at `max_attempts`, when the total deadline cannot accommodate another attempt, or immediately when the caller is canceled. Backoff waits use a reusable timer or context-aware wait rather than `time.Sleep`.

Do not retry caller cancellation, signed-out or non-ChatGPT authentication, unsupported model/configuration, policy refusal, malformed local input, or a schema-invalid successful response unless the implementation has an explicitly bounded corrective retry. Exhausted attempts mark the item failed and leave the source untouched. Attempt and terminal errors preserve their typed cause for `errors.Is`/`errors.As` while mapping to a stable user-safe code.

### 10.4 Availability behavior

Codex absence, authentication failure, allowance exhaustion, or processing failure must never prevent:
...
- Do not expose an OpenAI API-key setting. Deliberately ignore `OPENAI_API_KEY` and never inspect Codex's credential file or operating-system credential store.
- Invoke the configured executable directly with fixed App Server arguments; never concatenate it into a shell command. Inherited environment values may allow Codex to locate its own configuration, but the application must not enumerate or log them.
- Send only required image content and classification instructions through Codex. Restrict App Server sandbox read access to the transient rendition and deny workspace writes for analysis turns.
...
| Configuration-blocked | Codex command missing, signed out, API-key authentication, unsupported account/model | Serve existing catalog, mark new imports blocked, retain sources, and show setup guidance. |
| Retryable external | App Server interruption, timeout, available rate-limit reset, eligible upstream error | Retry with bounded backoff; retain on exhaustion. |
| Permanent external | Invalid or expired login that cannot refresh, refusal, unsupported input | Do not retry indefinitely; record failure and retain source. |
...
Tests use temporary application roots and never depend on the developer's real `incoming/`, `processed/`, configuration, browser, file manager, Codex installation, or ChatGPT account.
...
- Configuration creation and permissions, defaults, strict parsing, duplicate/unknown fields, non-overwrite behavior, Codex command/timeout bounds, loopback validation, and executable-relative paths (`FR-001`, `FR-002`).
- Codex preflight command resolution, fixed argument construction, initialization ordering, account-state classification, malformed/oversized messages, bounded notification skipping, cancellation, and child-process joining (`FR-003`, `FR-009`).
...
- Accept a valid mocked App Server turn with `localImage` input and structured output and preserve provenance (`FR-009`).
...
- Startup resolves the configured Codex command, completes the App Server handshake, reads account state, and reports ready only for ChatGPT authentication; every preflight child process is canceled and joined.
- Existing assets remain browsable and editable without Codex, a ChatGPT login, available allowance, or network connectivity.
...
| Codex App Server protocol changes | Pin a tested minimum Codex CLI release for distributions, keep the protocol adapter isolated, validate the initialization/account contract at startup, and fail processing closed while preserving catalog access. |
| ChatGPT allowance is exhausted during an import | Read rate-limit state, stop reserving new work, retain all sources, display reset guidance, and resume on a later application start. |
| A spawned Codex process outlives the application | Derive every process from application context, close pipes, cancel and join during preflight completion and shutdown, and test timeout/interrupt paths. |
