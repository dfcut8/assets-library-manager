...
4. Optionally copy and edit `config.example.json` as `config.json` in the same directory. Every field is optional and overrides a compiled default. Change `codex.command` only when `codex` is not available on `PATH`.
5. Run the binary.

When `config.json` is absent, the application runs directly from safe compiled defaults without creating the file. It creates `incoming/`, `processed/`, `processed/.staging/`, and a fully migrated `assets.db`, then launches a short-lived `codex app-server` preflight, listens at `http://127.0.0.1:7342/`, and opens the default browser. Set `server.open_browser` to `false` in `config.json` to disable browser launch.
...
On Windows, `make install` builds an optimized, stripped CGO-free Windows/amd64 executable with Git version metadata and installs only the binary in `D:\assets-library`. The target never creates, copies, or overwrites configuration; create `config.json` only when compiled defaults are insufficient.
